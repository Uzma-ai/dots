...
class FileSharingController extends Controller
{
    public function GetUrl(Request $request)
    {
        ...
        $roles = Roles::get();
        $users = User::get();
        $id = base64UrlDecode($request->filekey);
    ...
    public function FileView($id)
    {
        ...
        $type = substr($id, 8, 1) == 'i' ? "File" : "Folder";
        $decodedId = Hashids::decode(substr($id, 0, 8))[0];
        if ($type == "File") {
            $file = File::find($decodedId);
        } else {
            $file = Folder::find($decodedId);
        }
        if ($data) {
            $users = [];
            ...
            $roles = FileSharingRoles::where('file_sharing_id', $data->id)->get();
            ...
            $dbusers = FileSharingUsers::where('file_sharing_id', $data->id)->get();
            ...
            if (in_array($currentuser->id, $users)) {
                // return true;
                if ($data->files_id != null) {
                    $files = File::where('id', $data->files_id)->first();
                    $path = $files->path;
                    return view('sharing.fileview', compact('data', 'files', 'path', 'id'));
                } elseif ($data->folders_id != null) {
                    $files = Folder::where('id', $data->folders_id)->first();
                    $path = $files->path;
    ...
    public function pathfiledetail(Request $request)
    {
        ...
        if ($filepath != 'RecycleBin') {
            ...
            $folders = Folder::where('parentpath', $parentPath)->where('status', 1)->get();
    ...
    public function shareLinks($path=null)
    {
        ...
        ->get();
        $path = $path ? $path : '/';
         return view('sharing.sharelinks',compact('path','contextTypes','resizecontextTypes','sortcontextTypes', 'filteredPermissions'));
    }
