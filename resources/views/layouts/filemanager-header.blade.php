         ...
         <div
           ...
           <div class="hover-bg-c-yellow rounded-t-lg">
             <a href="#" class="flex block p-2 pl-4 dropdown-item clickmenu {{ $contextType->function }} " data-type="{{ $option->function }}">
               <img src="{{ asset($constants['FILEICONPATH'].($option->image ?? 'default').$constants['ICONEXTENSION'])}}" alt="{{ $option->name }}" class="pr-4 w-11" /><span>{{ $option->name }}</span>
             </a>
           </div>
           @endforeach
           @endif
         ...
         <div
           ...
           <div class="hover-bg-c-yellow rounded-t-lg">
             <a
               ...
               data-name="{{ $roption->name }}"
               onclick="toggleTick(this)">
               <i class="ri-check-line pr-3 ri-lg mt-1 tick-icon" style="display: none;"></i>
   ...
   <script>
     function toggleTick(clickedElement) {
       // Remove all tick marks
       document.querySelectorAll('.tick-icon').forEach(icon => {
         icon.style.display = 'none';
       });

       // Add tick mark to the clicked element
       const tickIcon = clickedElement.querySelector('.tick-icon');
       if (tickIcon) {
         tickIcon.style.display = 'inline'; // Show the tick icon
       }
     }
   </script>