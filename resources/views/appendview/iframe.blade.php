  ...
  <div class="draggable bg-opacity-70  shadow-lg w-full h-full relative">
    <div class="flex justify-between items-center p-1 pr-2 border-b bg-c-gray-gradient">
      ...
      <div class="flex space-x-1">
        ...
        <a href="#" class="closeiframe-btn" data-filekey="{{$iframedetail['filekey'] }}" data-iframe-id = "{{$iframedetail['filetype'].$iframedetail['filekey'] }}" data-appkey="{{ $iframedetail['appkey'] }}" data-filetype="{{ $iframedetail['filetype'] }}" data-apptype="{{ $iframedetail['apptype'] }}" ><img src="{{ asset($constants['IMAGEFILEPATH'].'close'.$constants['ICONEXTENSION'])}}"/></a>
    ...
    .then((data) => {
      ...
      /*console.log(messages)*/
        ...
        $.each(messages, function(index, comment) {
          ...
          <img src="${comment.user.avatar}" alt="Avatar" class="h-8 w-8 rounded-full" />
          </div>
          ...
          ${comment.replies.map(reply => `
            ...
            <div class="flex items-start gap-3">
            <div class="h-8 w-8 rounded-full">
            <img src="${reply.user.avatar}" alt="Avatar" class="h-8 w-8 rounded-full" />
            </div>
            <div class="flex-1 space-y-1">
            <div class="flex items-center justify-between">
