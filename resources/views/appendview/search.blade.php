 ...
 <h1 class="text-xl font-medium mb-2">{{ $filekey }}</h1>
  <ul>
      @foreach ($file as $filedet)
        <li class="flex items-center gap-4"><img class="w-8 h-8" src="{{ asset($constants['FILEICONPATH'].$filedet['extension'].'.png')}}" alt=""><p>{{ $filedet['name'] }}</p></li>
        @endforeach
 </ul>
</div>  
@endforeach
...
@endphp
        <li class="flex items-center gap-4"><img class="w-8 h-8" src="{{ asset($constants['FILEICONPATH'].'folder.png')}}" alt=""><p>{{ $folder->name }}</p></li>
@endforeach
 </ul>
</div>
